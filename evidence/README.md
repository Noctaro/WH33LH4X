...
the only route to this wheel's motor **on Windows**. Raw USB later turned out to be a second
route, and a better one: [`RAW_USB.md`](RAW_USB.md) is the write-up, and it is where the
interesting work now is. **The answers are below. The code is here so the answers can be
re-checked, not because anyone needs to run it.**
