...
- **Windows works too**, over WinUSB bound with Zadig, with the same generated bytes. No
  foreground gate there either: a sweep was identical with another application in front
  (2026-09-23).
- **It has driven a car.** DiRT 4 through vJoy, first on 2026-09-24 and again with the
  `bridge/` package on 2026-09-26, force direction measured correct from the logs.
- **Pedals and buttons are decoded**, in [`../gip/report.py`](../gip/report.py).
- **That is now the product**: `python -m bridge` and the window replaced the shim.

## Not done

- **A Linux game device.** `gip_wheel_driver.py` publishes steering only and takes no force
  from games; the bridge on Linux runs its spring with no virtual device yet.
