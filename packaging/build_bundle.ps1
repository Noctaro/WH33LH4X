...
#
# PyInstaller wraps a program in an extract-and-run bootloader, the pattern packers use, and
# antivirus objects to it. Embeddable CPython ships a python.exe SIGNED BY MICROSOFT, keeps the
# source readable so a suspicious user can check what it does, and lets someone edit a line and
# send back a log. None of that survives a frozen bundle.
#
...
#
# One resolved pass installs the pinned set together. Separate --target passes overwrite rather
# than merge, which breaks any namespace package two wheels contribute to.
#
...
# script library. The window needs all three. They come from the host CPython install, which is
...
# '..' is the bundle root (our packages) and '..\lib' the vendored dependencies.
...
# ONE invocation, resolving the whole pinned set together. See the note above.
...
foreach ($d in $sourceDirs) {
    ...
    if (-not (Test-Path $src)) { Write-Error "Expected folder missing from the repo: $d" }
...
# --- launchers -----------------------------------------------------------------------------
# The bridge alone, in a console, for running without the window. %~dp0 keeps the folder
# relocatable; the ._pth above puts the bundle root on sys.path, so -m finds the packages.
$cmdLines = @(
    '@echo off',
    '"%~dp0python\python.exe" -m bridge %*'
)
...
# the launching cmd closes instead of lingering for the life of the window.
$guiLines = @(
    '@echo off',
    'start "" "%~dp0python\pythonw.exe" -m ui %*'
)
...
# they extracted and a bug report cannot name one. The window shows it in its title bar.
...
Write-Host "  $staging\python\python.exe -c ""import usb, libusb_package, pyvjoy, sv_ttk; print(libusb_package.get_library_path())"""
